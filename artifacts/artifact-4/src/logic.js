...
/* 2. INVENTARDATEN
   sharedInventory → normales Vorratslevel
*/
...
/* 3. APP-STATE
   ...
   Alle Nutzerentscheidungen werden hier gespeichert.
*/
const appState = {
    mode:           null,     // "single", "multi" oder null (Start)
    source:         "all",    // "all", "saved", "history"
    selectedRecipe: null,     // Name des gewählten Rezepts (null ist nichts)
    people:         9,        // Personenanzahl
    days:           3,        // Tagesanzahl
    buffer:         1,        // Puffertage
    saved:          false,    // Wurde gespeichert?
    shared:         false     // Wurde geteilt?
};
...
// Gibt alle passenden Elemente als Liste zurück
function getElements(selector) {
    return document.querySelectorAll(selector);
}

// Formatiert Zahlen leserlich:
// Ganze Zahlen ohne Nachkommastellen und Dezimalzahlen mit zwei Stellen
...
// Die Feasibility-Screens teilen sich zwischen Single und MultiFlow.
...
function showScreen(screenName) {
    ...
    // Damit "Adjust servings / days" nur im Multi-Modus angezeigt wird.
...
/* 6. INPUTS LESEN
   ...
   und schreibt sie in den App-State. (Gehirn)
   */
function updateStateFromInputs() {
    // Single meal: fester Plan für 1 Person, 1 Tag, kein Puffer.
    // Die Multi-Eingaben werden im Single-Modus ignoriert.
...
/* 7. REZEPT WÄHLEN
   ...
   Speichert das gewählte Rezept im App-State.
   */
function setSelectedRecipe(recipeName) {
    ...
    // State aktualisieren, Rezept abspeichern.
...
/* 9. UI AKTUALISIEREN
   ...
   Werte in die richtigen HTML-Elemente.
   */
...
function updateOverview() {
    ...
    // Kein Rezept gewählt, nichts aktualisieren
    if (!recipe) return;
...
/* 
   10. AKTIONEN
   Reaktionen auf konkrete Nutzeraktionen (Buttons mit Aktionen).
 */

// VERGLEICH: Prüft Machbarkeit und wechselt zum passenden Screen.
// feasible = true → grüner Screen, wenn false → roter Screen
